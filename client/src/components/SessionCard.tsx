  ...
  return (
    <div
      className={`grid grid-cols-1 md:grid-cols-2 gap-8 items-center py-12 ${
        !isEven ? (isRtl ? "md:[direction:ltr]" : "md:[direction:rtl]") : (isRtl ? "md:[direction:rtl]" : "md:[direction:ltr]")
      }`}
      dir={isRtl ? 'rtl' : 'ltr'}
    >
      {/* Content Side */}
      <div className={`${isEven ? "md:col-start-1" : "md:col-start-2"} ${isRtl ? 'text-right' : 'text-left'}`} dir={isRtl ? 'rtl' : 'ltr'}>
        <div className="space-y-4">
          ...
          <div className="bg-gradient-to-br from-primary/5 to-secondary/5 p-4 rounded-lg border border-border">
            <h3 className="font-semibold text-foreground mb-2">{t('whyItMatters')}</h3>
            <ul className="space-y-1">
              {content.why.map((item: string, i: number) => (
                <li key={i} className={`text-sm text-foreground flex items-start gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
          ...
          <button
            ...
            className={`w-full flex items-center justify-between p-3 bg-secondary/10 hover:bg-secondary/20 rounded-lg transition-colors border border-border ${isRtl ? 'flex-row-reverse' : ''}`}
          >
            <span className="font-semibold text-foreground">{t('viewLessons')}</span>
            <ChevronDown
          ...
          {isExpanded && (
            <div className="space-y-4 pt-4 border-t border-border">
              {/* Example */}
              <div>
                <h4 className="font-semibold text-foreground mb-2">{t('example')}:</h4>
              ...
              {/* Mistakes */}
              <div>
                <h4 className="font-semibold text-foreground mb-2">{t('commonMistakes')}:</h4>
                <ul className="space-y-1">
                  {content.mistakes.map((mistake: string, i: number) => (
                    <li key={i} className={`text-sm text-destructive flex items-start gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
              ...
              {/* Tip */}
              <div className={`bg-accent/10 p-3 rounded-lg border border-accent/30 flex items-start gap-2 ${isRtl ? 'flex-row-reverse' : ''}`}>
                <span className="text-sm font-semibold text-accent-foreground mt-0.5">💡</span>
              ...
              <div className="bg-primary/10 p-3 rounded-lg border border-primary/30">
                <p className="text-sm text-foreground">
                  <span className="font-semibold">{t('miniActivity')}:</span> {content.task}
                </p>
              </div>
      ...
      {/* Visual Side */}
      <div className={`hidden md:flex items-center justify-center ${isEven ? "md:col-start-2" : "md:col-start-1"}`} dir="ltr">
